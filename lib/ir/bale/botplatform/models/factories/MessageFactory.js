...
const LocationMessage_1 = require("../message/LocationMessage");
const ContactMessage_1 = require("../message/ContactMessage");
class MessageFactory {
    ...
    static build(message) {
        switch (message.$type) {
            ...
            case ApiConst_1.ApiConst.MessageTypes.JsonMessage:
                    let data = JSON.parse(message.rawJson);
                    let dataType = data.dataType;
                    switch (dataType) {
                        case (ApiConst_1.ApiConst.JsonTypes.Location):
                            return new LocationMessage_1.LocationMessage(data.data);
                        case (ApiConst_1.ApiConst.JsonTypes.Contact):
                            return new ContactMessage_1.ContactMessage(data.data);
                        default:
                            return null;
                    }
