...
describe('SendTextMessageRequest', function () {
    it('It should return correct string request!', function () {
        ...
        let user = new User_1.User(1374, "741115");
        let textMsgReq = JSON.parse(new SendMessageRequest_1.SendMessageRequest(new TextMessage_1.TextMessage("MyMessage"), user).toJson());
    ...
    it('It should return real random randomId!', function () {
        let user = new User_1.User(1374, "741115");
        let user2 = new User_1.User(1374, "131415");
        let textMsgReq = new SendMessageRequest_1.SendMessageRequest(new TextMessage_1.TextMessage("MyMessage"), user);
        let textMsgReq2 = new SendMessageRequest_1.SendMessageRequest(new TextMessage_1.TextMessage("MyMessage2"), user2);
    ...
    it('It should return real random id!', function () {
        ...
        let user2 = new User_1.User(1374, "131415");
        let textMsgReq = JSON.parse(new SendMessageRequest_1.SendMessageRequest(new TextMessage_1.TextMessage("MyMessage"), user).toJson());
        let textMsgReq2 = JSON.parse(new SendMessageRequest_1.SendMessageRequest(new TextMessage_1.TextMessage("MyMessage2"), user2).toJson());
        expect(textMsgReq.id).to.not.equal(textMsgReq2.id);
    });
});
//# sourceMappingURL=SendTextMessageRequestTest.js.map